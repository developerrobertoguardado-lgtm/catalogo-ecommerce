...
2. Con la respuesta, pregunta en cascada (una a la vez):
   - ¿Qué problema resuelve / para qué usuario? (visitante del catálogo o
     administrador)
   - ¿Qué debe pasar exactamente? (criterios de aceptación, casos borde)
   - ¿Toca datos nuevos? (nuevas entidades/campos sobre `Product`,
     `Category`, `ProductImage`, `Order`, `OrderItem`, `StoreSetting`)
   - ¿Toca rutas nuevas? (método, ruta, si es pública o de `/admin`, request/
     response esperado — ver convención en `/spec/04-api-contracts.md`)
...
4. Calcula el correlativo: revisa las carpetas existentes en
   ...
   (3 dígitos, con ceros a la izquierda: `005`, `006`...). Si no hay ninguna
...
7. Antes de dar por cerrada la sesión (ya sea que solo se creó la spec, o que
   ...
   siguiendo el formato ya usado en ese archivo. Usa `{{tipo}} = Feature` y
   menciona la carpeta `{{NNN}}-{{slug}}` creada. Si solo quedó la spec creada
   (sin implementar todavía), marca **Estado: ⚠️ Parcial** y en "Siguiente
   paso sugerido" indica que falta implementarla.
